# Komiic Source Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement komiic.com as a GraphQL-based MangaSource plugin for comic-reader.

**Architecture:** Pure GraphQL JSON API. All requests are `POST /api/query` with JSON body. No HTML parsing needed. Images accessed via `/api/image/{kid}` with per-chapter Referer headers.

**Tech Stack:** Dart/Flutter, Dio HTTP client, dart:convert for JSON

## Global Constraints

- Follow MangaSource prepare/parse separation pattern
- Sources never touch HttpClient directly
- All imports use `package:comic_reader/...` absolute paths
- Use `const` constructors where possible
- `flutter analyze` must pass with no errors
- Chapter IDs are string representations of komiic's numeric IDs

---

### Task 1: Source Scaffold + DI Registration

**Files:**
- Create: `lib/data/sources/komiic.dart`
- Modify: `lib/app/di/injection.dart`

**Interfaces:**
- Produces: `Komiic` class extending `MangaSource`, with all property getters, `discoveryFilters`, and all method pairs throwing `UnimplementedError()`

- [ ] **Step 1: Create komiic.dart with class scaffold**
...
class Komiic extends MangaSource {
  ...
  String _buildGraphqlBody(String operationName, String query, Map<String, dynamic> variables) {
    return jsonEncode({
...
- [ ] **Step 2: Register in injection.dart**

Add import at top of `lib/app/di/injection.dart`:
```dart
import 'package:comic_reader/data/sources/komiic.dart';
```

Add registration after line 80 (after `registry.register(IkanManhua());`):
```dart
...
Run: `flutter analyze lib/data/sources/komiic.dart`
Expected: No errors (warnings about unused imports/fields acceptable)

- [ ] **Step 4: Commit**

```bash
git add lib/data/sources/komiic.dart lib/app/di/injection.dart
git commit -m "feat(komiic): add source scaffold with filters and DI registration"
```
...
**Files:**
- Modify: `lib/data/sources/komiic.dart`

**Interfaces:**
- Consumes: `_buildGraphqlBody` helper from Task 1
- Produces: Working `prepareDiscoveryFetch(int page, Map<String, String> filters)` → `FetchConfig` and `parseDiscovery(dynamic response)` → `List<MangaSummary>`
...
- [ ] **Step 2: Implement parseDiscovery**

Replace the `parseDiscovery` method:
...
List<MangaSummary> _parseComicsList(dynamic response) {
  ...
  // Handle both direct 'comics' key and nested structures
...
```bash
git add lib/data/sources/komiic.dart
git commit -m "feat(komiic): implement discovery with hotComics/recentUpdate/comicByCategories"
```
...
**Interfaces:**
- Consumes: `_buildGraphqlBody`, `_parseComicsList` from Tasks 1-2
- Produces: Working `prepareSearchFetch(String keyword, int page, Map<String, String> filters)` → `FetchConfig` and `parseSearch(dynamic response)` → `List<MangaSummary>`
...
List<MangaSummary> parseSearch(dynamic response) {
  ...
  final searchResult = data['searchComicsAndAuthors'] as Map<String, dynamic>?;
...
```bash
git add lib/data/sources/komiic.dart
git commit -m "feat(komiic): implement search with searchComicsAndAuthors"
```

---

### Task 4: MangaInfo + ChapterList

**Files:**
- Modify: `lib/data/sources/komiic.dart`

**Interfaces:**
- Consumes: `_buildGraphqlBody` from Task 1
- Produces: Working `prepareMangaInfoFetch` / `parseMangaInfo` / `prepareChapterListFetch` / `parseChapterList`
...
- [ ] **Step 3: Implement prepareChapterListFetch**

Replace the `prepareChapterListFetch` method:
...
- [ ] **Step 4: Implement parseChapterList**

Replace the `parseChapterList` method:
...
ChapterListResult parseChapterList(dynamic response, String mangaId) {
  ...
  final chapterItems = items.map<ChapterItem>((item) {
    ...
    final title = isBookType ? '第${serial}卷' : '第${serial}話';
...
```bash
git add lib/data/sources/komiic.dart
git commit -m "feat(komiic): implement manga info and chapter list"
```
...
**Interfaces:**
- Consumes: `_buildGraphqlBody` from Task 1
- Produces: Working `prepareChapterFetch` / `parseChapter` with correct image Referer headers
...
- [ ] **Step 3: Override getChapterWebUrl**

Add this method to provide proper browser URL:
...
```bash
git add lib/data/sources/komiic.dart
git commit -m "feat(komiic): implement chapter image loading with Referer headers"
```
...
**Files:**
- Read: `lib/data/sources/komiic.dart` (full file review)
- Read: `lib/app/di/injection.dart` (verify registration)

**Interfaces:**
- Consumes: All previous tasks
- Produces: Verified, passing source implementation

- [ ] **Step 1: Run full static analysis on the source file**
...
Run: `flutter analyze`
Expected: No errors from komiic.dart

- [ ] **Step 3: Verify the complete file is internally consistent**

Review complete source file for:
- All methods implemented (no `UnimplementedError` remaining)
- `dart:convert` imported for `jsonEncode`/`jsonDecode`
- Consistent use of `sourceId` vs `id`
- No typos in GraphQL query strings
- Image Referer format matches verified working pattern

- [ ] **Step 4: Final commit (if any fixups needed)**

```bash
git add -A
git commit -m "feat(komiic): final verification and cleanup"
```
