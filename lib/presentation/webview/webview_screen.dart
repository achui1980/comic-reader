...
class _WebViewScreenState extends State<WebViewScreen> {
          ...
          Expanded(
            child: InAppWebView(
              ...
              onWebViewCreated: (controller) {
                ...
                // Register JS handler to receive data from injected script
              ...
              onLoadStop: (controller, url) async {
                // After page loads, inject JS to extract cookies
  ...
  Future<void> _injectCookieExtractor(InAppWebViewController controller) async {
    ...
    // If source has custom injected JS, use that
    if (source?.injectedJavaScript != null) {
      await controller.evaluateJavascript(source: source!.injectedJavaScript!);
      return;
    }

    // Default: extract document.cookie and post it back
    await controller.evaluateJavascript(source: '''
      (function() {
        try {
          var cookie = document.cookie || '';
          var title = document.title || '';
          // Only send if we're past the CF challenge
